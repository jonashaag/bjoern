...
def bottle_app():
    ...
    @app.get("/a/b/c")
    def hello():
        return "Hello, World! {}".format(sorted(request.params.items()))

    @app.post("/a/b/c")
    def hello():
        return "Hello, World! {} {}".format(
            sorted(request.params.items()), sorted(request.forms.items())
        )
...
def test_bottle_app(bottle_app, client):
    ...
    assert (
        response.content
        == b"Hello, World! [('k', 'v'), ('k2', \"v2'\"), ('k3', 'v3'), ('k4', 'v4')] [('k3', 'v3'), ('k4', 'v4')]"
    )
